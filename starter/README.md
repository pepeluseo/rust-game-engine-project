...
```bash
sudo apt update
sudo apt install -y build-essential git make pkg-config libglfw3 libglfw3-dev libx11-dev libxi-dev libxcursor-dev libxrandr-dev libxinerama-dev libgl1-mesa-dev
```
...
The original C test game can be executed from the `starter` directory:

```bash
make run-c
```

This builds the C OpenGL wrapper and launches a test window.

Expected behavior:

```text
A C Test Game window opens and renders visible sprites.
```

Example terminal output may include graphical warnings in WSL:
